...
//    });
// }
// list=someFunction(array)

  // const seasons=[winter,fall,summer.spring]
let list=array.map(el=>el)
// console.log(newA)



  
  array.sort((a,b)=>new Date(b.cohort.cohortStartDate)-new Date(a.cohort.cohortStartDate))
  ...
  if(event.target.id===`all-students-button`){
    ...
    let filteredArray=[...data.filter(el=>event.target.id===el.cohort.cohortCode)]
    let cohort=event.target.id.match(/[a-zA-Z]/gi).join(``)+` `+event.target.id.match(/[0-9]/gi).join(``)
...
//   function addComment(){
//     setCommentList([changeText,...data])
      ...
      <main style={{backgroundColor:backgroundColor,color:color}}>
        ...
        <div className="cohort-list-container">
                <ul className="cohort-list">
                  ...
                  <button id='all-students-button'className="all-students-button" onClick={handleList}> All Students</button>

        {...array.map((student) => {
          return (
          
                  <CohortList handleList={handleList} student={student} generateNewColor={generateNewColor}/>
...
<p className="total-students">Total Students: <span>{NumberOfStudent}</span></p>
<button type="mode" onClick={toggleMode}> {color===`black`?`Dark Mode`:`Light Mode`}</button>
        ...
        {...newList.map((student,i) => {
          return (
                  ...
                  <StudentCard student={student} addComment={addComment} showMore={showMore} toggleShow={toggleShow} key={student.id+i}/>
